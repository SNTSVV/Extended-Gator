...
public class Hierarchy {
  ...
  public boolean isFragmentClass(final SootClass c) {
    return isSubclassOf(c, Scene.v().getSootClass("androidx.fragment.app.Fragment"))
            || isSubclassOf(c, Scene.v().getSootClass("android.support.v4.app.Fragment"))
            || isSubclassOf(c, Scene.v().getSootClass("android.app.Fragment"));
  }
...
//      }
      ...
      if (!applicationActivityClasses.contains(c) && !applicationFragmentClasses.contains(c)) {
        //Logger.verb(this.getClass().getSimpleName(),"Not application activity and fragment class: "+c.getName());
        continue;
      }
  ...
  void fragmentClasses() {
    ...
    SootClass frag3 = Scene.v().getSootClass("android.app.Fragment");
    if (frag1 == null && frag2 == null && frag3 == null){
      throw new Error("[HIER] Did not find Fragment");
    }
    for (SootClass c: appClasses) {
      if (getSupertypes(c).contains(frag1) || getSupertypes(c).contains(frag2) || getSupertypes(c).contains(frag3) ){
  ...
  void viewsAndMenus() {
    ...
    SootClass viewHolder = scene.getSootClass("android.support.v7.widget.RecyclerView$ViewHolder");
    ...
    for (SootClass c : appClasses) {
      ...
      if (superClzSet.contains(view) || superClzSet.contains(viewHolder)) {
        numAppViews++;
        viewClasses.add(c);
      }

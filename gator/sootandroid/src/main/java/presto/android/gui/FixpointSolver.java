...
public class FixpointSolver {
  ...
  public Map<NNode, SootClass> inflateProducer;
  ...
  public void preSolveInit() {
    ...
    inflateProducer = Maps.newHashMap();
  }
...
//        }
  ...
  void solutionReceiverReachability() {
    for (Map.Entry<NOpNode, Set<NNode>> entry : reachingReceiverViews.entrySet()) {
      NOpNode key = entry.getKey();
      Logger.verb("DEBUG","[SolutionReceiver] key: "+key);
      Set<NNode> value = solutionReceivers.get(key);
      for (NNode n : entry.getValue()) {
        Logger.verb("DEBUG","[SolutionReceiver] value: "+key);
        if (!(n instanceof NObjectNode)) {
          continue;
        }
  ...
  void solutionListenersReachability() {
    ...
    for (Map.Entry<NOpNode, Set<NNode>> entry : reachingListeners.entrySet()) {
      ...
      Logger.verb("FixpointSolver", "Set Listerner: " + setListener.callSite);
      ...
      for (NNode n : reachables) {
        ...
        Logger.verb("FixpointSolver", "Listerner Object: "+ n.toString());
    ...
    for (NLayoutIdNode layoutIdNode : flowgraph.allNLayoutIdNodes.values()) {
      for (NNode target : graphUtil.reachableNodes(layoutIdNode)) {
        ...
        if (opNode.consumesLayoutId()) {
          ...
          //Logger.verb("DEBUG", "Opnode: " + opNode.toString() + "ReachingLayoutIds: "+layoutIdNode);
        } else {
  ...
  void parameterAndReceiverViewReachability(NNode source) {
    ...
    {
      Logger.verb("DEBUG", source + " is not reachable");
    }
    for (NNode target : reachables) {
      if (!(target instanceof NOpNode)) {
        continue;
      ...
      if (opNode instanceof NAddView1OpNode
        ...
        //Logger.verb("DEBUG", "Add to reachingParameterViews -> "+opNode);
      } else {
        // Maybe SetListener
    ...
    for (Map.Entry<NOpNode, Set<NLayoutIdNode>> entry : reachingLayoutIds.entrySet()) {
      ...
      for (NLayoutIdNode layoutIdNode : entry.getValue()) {
        if (opNode instanceof NInflate1OpNode) {
          Logger.verb("DEBUG", "Do Layout Inflate " + opNode +" for: "+ layoutIdNode);
          NInflNode inflNode = doLayoutInflate(layoutIdNode.getIdValue(), null, null);

          if (inflNode == null) {
            //Logger.verb("DEBUG", "InflateNode: Null");
            continue;
          }
  ...
  private void registerInflateNode(NInflNode inflNode, SootClass declaringClass) {
    //Logger.verb("DEBUG", "InflateNode: "+inflNode + " create in: "+declaringClass.getName());
    inflateProducer.put(inflNode,declaringClass);
    for (NNode child: inflNode.getChildren())
    {
      if (child instanceof NInflNode)
      {
        registerInflateNode((NInflNode) child, declaringClass);
      }
    }
  }
  ...
  void inflationEffectsToParameters(NOpNode opNode, NInflNode inflNode,
    ...
    if (opNode.id == 3403)
    {
      Logger.verb("inflationEffectsToParameters",opNode.toString());
    }
    for (Map.Entry<NOpNode, Set<NOpNode>> parameterAndCalls : reached.entrySet()) {
      ...
      for (NOpNode call : calls) {
        ...
        if (opNode.id == 3403)
        {
          Logger.verb("inflationEffectsToParameters",call.toString());
        }
  ...
  void inflationEffectsToReceivers(NOpNode opNode, NInflNode inflNode) {
    ...
    for (Map.Entry<NOpNode, Set<NOpNode>> receiverAndCalls : reachedReceiverViews.entrySet()) {
      ...
      for (NOpNode call : calls) {
        ...
        //DEBUG
       /* if (call instanceof NSetListenerOpNode)
        {
          Logger.verb("FixpointSolver", "Set Listener: " + call.toString() + "--> InflNode: "+inflNode.toString());
        }*/
        solutions.add(inflNode);
      }
    }
  ...
  void inflNodeToReceiverAndParameter(NNode inflNode) {
    ...
    for (NNode n : reachables) {
      ...
      if (opNode instanceof NFindView1OpNode
        ...
        {
          //Logger.verb("FixpointSolver", "Set Listener: " + opNode.toString() + "--> InflNode: "+inflNode.toString());
        }
      }
    }
  }
  ...
  /**
   * Author: NGO Chanh Duc
   * @param inflNode
   */
  void inflNodeToReceiverAndParameterAlongWithWindow(NNode inflNode, NNode window) {
    Set<NNode> reachables = graphUtil.reachableNodes(inflNode);
    Set<NNode> windowReachables = graphUtil.reachableNodes(window);
    for (NNode n : reachables) {
      if (!(n instanceof NOpNode)) {
        continue;
      }
...
//        vNode = flowgraph.inflNode(v.getSootClass());
      ...
      if (text != null) {
        Logger.verb("DEBUG","Menu item text: "+text);
  ...
  NInflNode doLayoutInflate(Integer layoutId, NNode rootparent, NVarNode inflateOpNodeReciever) {
    ...
    while (!worklist.isEmpty()) {
      AndroidView v = worklist.remove();
      NNode parent = nodeWorklist.remove();
      //Logger.verb("DEBUG", "AndroidView: " + v.toString());
  ...
  void viewAndListenerPropagation() {
    while (true) {
      ...
      //Logger.verb("DEBUG","[FixpointSolver] NFindView20pNode size: "+NOpNode.getNodes(NFindView2OpNode.class).size());
      ...
      //Logger.verb("DEBUG","[FixpointSolver] NFindView3OpNode size: "+NOpNode.getNodes(NFindView3OpNode.class).size());
      for (NOpNode findView3 : NOpNode.getNodes(NFindView3OpNode.class)) {
        if (processFindView3((NFindView3OpNode) findView3)) {
          changed = true;
        }
      }
      //Logger.verb("DEBUG","[FixpointSolver] NAddView1OpNode size: "+NOpNode.getNodes(NAddView1OpNode.class).size());
...
//          Logger.verb(this.getClass().getSimpleName(), "  * lhs-item: " + lhs);
  ...
  boolean processFindView2(NFindView2OpNode node) {
    ...
    int oldSize = solution.size();
//    if (windows.size()>50) {
//      Logger.verb("DEBUG", "[FixpointSolver] Reaching windows size: " + windows.size());
//      Logger.verb("DEBUG", "[FixpointSolver] Ignore this node " + windows.size());
//      return false;
//    }
...
//      if (windows.size()>50)
//      {
//        Logger.verb("DEBUG", "[FixpointSolver] Activity: " + window.toString() );
//      }
      for (NNode root : window.getChildren()) {
        //Logger.verb("DEBUG", "[FixpointSolver] root descendant size: " + graphUtil.descendantNodes(root).size() );
 ...
 /*     for (NNode node1: windowReachables) {
  ...
  private Boolean containedInWindow(NNode window, NNode parent, Set<NNode> processedChilds) {
    if (window == parent)
      return true;
    for (NNode child: window.getChildren()){
      if (processedChilds.contains(child))
        continue;
      processedChilds.add(child);
      if (containedInWindow(child,parent,processedChilds))
        return true;
    }
    return false;
  }
  ...
  void propagateToParametersAndReceivers(Set<NNode> solution, NOpNode node) {
    ...
    if (callsWithReceiver != null && !callsWithReceiver.isEmpty()) {
      for (NOpNode call : callsWithReceiver) {
        ...
        Boolean debugCondition = call instanceof NSetListenerOpNode & call.toString().contains("PlayerActivity");
        Set<NNode> trueSolution = Sets.newHashSet();
        for (NNode s : solution) {
        ...
        if (call instanceof NSetListenerOpNode)
        {
          for (NNode nnode : trueSolution) {
            //Logger.verb("Propagate", "Set Listener: "+call.toString()+"--> node: "+ nnode.toString());
          }

...
//              Logger.verb("AnalyzeCallback", "Handlers: "+bean.getHandlers().size());
  ...
  private void buildWTGEdges(
    ...
    if (mainClz == null) {
      Logger.verb("WTGEdges", "No main activity");
      return;
    }
    Logger.verb("WTGEdges", "Main activity: "+mainClz.getName());
  ...
  private Multimap<NObjectNode, HandlerBean> buildWidgetHandlers(
    ...
    for (NObjectNode window : guiHierarchy.keySet()) {
      ...
      if (window instanceof NActivityNode) {
        ...
        if (om != null) {
          SootMethod callback = wtgHelper.getCallback(window.getClassType(),
                  MethodNames.onCreateOptionsMenu1Subsig);
          if (callback != null) {
  ...
  private void buildActivityForwardEdge(
    ...
    if (targetWindow instanceof NActivityNode) {
      ...
      */
      ...
      /*
      WTGEdge newEdge = null;
      if (!subsig.equals(MethodNames.onCreateOptionsMenu1Subsig)) {
